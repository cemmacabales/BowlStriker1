...
import { ShoppingBag, User, Menu, X, Bot, Search } from 'lucide-react';
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-[#0D0D0D]/80 backdrop-blur-xl">
      ...
      <div className="md:hidden bg-[#0D0D0D]/95 backdrop-blur-xl border-b border-white/10">
          ...
          <Link
            key={link.path}
            to={link.path}
            className={`block px-3 py-3 rounded-lg text-base font-medium ${isActive(link.path) ? 'bg-white/10 text-primary-cyan' : 'text-white/70 hover:bg-white/5 hover:text-white'}`}
